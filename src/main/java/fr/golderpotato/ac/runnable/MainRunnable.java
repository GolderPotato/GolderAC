...
public class MainRunnable extends BukkitRunnable{
    ...
    public void run() {
        for(GACPlayer gplayers : Main.getInstance().getGACPlayers().values()){
            ...
            final int timer = (int)CheatType.TIMER.getValue("alert");
    ...
    public static boolean isLagging(final GACPlayer player){
    ...
    public static String lag(final GACPlayer player){
        if(player.pings.size() < 10){
            return "INVALID";
        }
        int pingtotal = 0;
