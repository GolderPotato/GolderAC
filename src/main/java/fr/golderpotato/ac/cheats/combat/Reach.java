    ...
    public void setupListener() {
        GACPackets.getInstance().addPacketListener(new GACPacketHandler(PacketType.USE_ENTITY) {
            ...
            public Packet Receive(final Packet paramPacket) {
                ...
                if(!(ent instanceof LivingEntity))return paramPacket;
                final LivingEntity damaged = (LivingEntity)ent;

                final Location entityLoc = damaged.getLocation().add(0.0D, damaged.getEyeHeight(), 0.0D);
                final Location playerLoc = player.getLocation().add(0.0D, player.getEyeHeight(), 0.0D);
                final double distance = PositionUtils.getDistance(entityLoc, playerLoc);
